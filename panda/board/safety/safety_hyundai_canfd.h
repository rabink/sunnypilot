...
const int HYUNDAI_PARAM_CANFD_UPSTREAM_TACO = 2048;
bool hyundai_canfd_alt_buttons = false;
bool hyundai_canfd_hda2_alt_steering = false;
bool hyundai_canfd_upstream_taco = false;
...
static int hyundai_canfd_tx_hook(CANPacket_t *to_send) {
  ...
  if (addr == steer_addr) {
    ...
    // 2m/s margin
    if (hyundai_canfd_upstream_taco &&
        ((hyundai_canfd_front_left_vego < (11.f + 2.f)) || (hyundai_canfd_rear_right_vego < (11.f + 2.f)))) {
