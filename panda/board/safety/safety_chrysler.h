...
const uint32_t CHRYSLER_PARAM_OFFSET = 8U;
const uint32_t CHRYSLER_PARAM_RAM_DT = 1U;  // set for Ram DT platform
const uint32_t CHRYSLER_PARAM_RAM_HD = 2U;  // set for Ram DT platform
const uint32_t CHRYSLER_PARAM_RAM_HD_S0 = 1U << CHRYSLER_PARAM_OFFSET;
...
static int chrysler_fwd_hook(int bus_num, int addr) {
  ...
  bool block_msg = is_cruise_buttons;
  // forward to camera
  if (bus_num == 0) {
    if (addr == chrysler_addrs->ESP_8) {
      bus_fwd = 2;
    } else if ((is_cruise_buttons || is_lkas_button) && (chrysler_platform == CHRYSLER_RAM_HD_S0)) {
      bus_fwd = 1;
    } else {
